...
public class UserController {
    ...
    public Response login(@RequestParam("username") String username,
        ...
        if (userInfo == null){
            return Response.USERNAME_PASSWORD_INVALID;
        }else if (!userInfo.getPassword().equalsIgnoreCase(md5(password))){
            return Response.USERNAME_PASSWORD_INVALID;
        }
    ...
    private String genToken() {
        return randomCode("0123456789abcdefghijklmnopqrstuvwxyz",13);
    }

    private String randomCode(String code,int size) {
        StringBuilder result = new StringBuilder(size);
        Random random = new Random();
        for (int i = 0 ; i < size; i++){
            int loc = random.nextInt(code.length());
            result.append(code.charAt(loc));
        }
